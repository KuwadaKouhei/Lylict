...
interface MindmapState {
  ...
  isLoading: boolean;
  error: string | null;
  // 世代フィルタ機能（配列でシリアライズ可能に）
  visibleGenerations: number[];
  highlightedGeneration: number | null;
}
...
const mindmapSlice = createSlice({
  ...
  reducers: {
    ...
    createAutoGeneratedMindMap: (state, action: PayloadAction<{
      ...
      state.visibleGenerations = Array.from(generations);
      state.highlightedGeneration = null;
    },
    clearError: (state) => {
      state.error = null;
    },
    // 世代フィルタ関連のアクション
    setGenerationVisibility: (state, action: PayloadAction<{ generation: number; visible: boolean }>) => {
      const { generation, visible } = action.payload;
      const currentGenerations = new Set(state.visibleGenerations);
      
      if (visible) {
        currentGenerations.add(generation);
      } else {
        currentGenerations.delete(generation);
      }
      
      state.visibleGenerations = Array.from(currentGenerations);
    },
    setAllGenerationsVisibility: (state, action: PayloadAction<{ generations: number[]; visible: boolean }>) => {
      const { generations, visible } = action.payload;
      state.visibleGenerations = visible ? generations : [];
    },
    setHighlightedGeneration: (state, action: PayloadAction<number | null>) => {
      state.highlightedGeneration = action.payload;
    },
    initializeGenerationFilters: (state) => {
      // 現在のノードから世代を抽出して、すべて表示状態で初期化
      const generations = new Set<number>();
